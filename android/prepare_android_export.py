#!/usr/bin/env python3
"""Prepare upstream SMB1R 1.1 for a reproducible ARM64 Android export."""
...
def download_file_picker_aars() -> None:
    ...
    for filename in ("app-debug.aar", "app-release.aar"):
        ...
        urllib.request.urlretrieve(
            f"{OLD_ANDROID_RAW}/addons/AndroidFilePicker/{filename}", destination
        )
...
def patch_project_settings() -> None:
    ...
    # Discord RPC is a desktop GDExtension and cannot be loaded on Android.
    text = text.replace("use_discord=true", "use_discord=false")
    text = text.replace(', "res://addons/discord-rpc-gd/plugin.cfg"', "")
    text = text.replace('"res://addons/discord-rpc-gd/plugin.cfg", ', "")

    # Enable the native Android file picker plugin.
    ...
    # The root Window must expose the complete landscape width. The Android
    # runtime wrapper then centers the selected game viewport inside it.
    ...
    # Godot 4.6 validates Android ETC2/ASTC texture imports during export.
    ...
    if etc2_setting not in text:
        ...
        if rendering_marker in text:
            text = text.replace(
                rendering_marker, rendering_marker + "\n" + etc2_setting + "\n", 1
            )
        else:
            text = text.rstrip() + "\n\n[rendering]\n\n" + etc2_setting + "\n"
