...
const LISTENER_PROBE_VERSION := 6
...
func _probe_level_theme_listeners() -> bool:
	...
	if previous_version < LISTENER_PROBE_VERSION and previous_status in ["running", "complete"]:
		print("[ANDROID_TITLE_READY_PROBE] Rearming v6 LevelBG probe from old checkpoint")
		previous = {}
		if not _write_listener_checkpoint({"status": "rearmed_probe_v6", "probe_version": LISTENER_PROBE_VERSION}):
			await _mark("V ERROR", "Could not rearm listener probe v6", 3600.0)
			return false
	...
	for group_start in range(0, connections.size(), LISTENER_GROUP_SIZE):
		...
		for i in range(group_start, group_end):
			...
			var state := {"status": "running", "phase": "before", "index": i,
				...
				"probe_version": LISTENER_PROBE_VERSION}
			...
			if not _write_listener_checkpoint(state):
				...
				return false
	if not _write_listener_checkpoint({"status": "complete", "count": connections.size(), "probe_version": LISTENER_PROBE_VERSION}):
